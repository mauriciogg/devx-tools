...
// Conn implements the net.Conn interface on top of a qemu_pipe, communicating
// close events over the control socket.
...
// Close closes the connection, this will send
// out a close event on the control socket.
...
// closeConn waits for the read end and the write end of the connection
...
type ControlSocket struct {
	...
	available chan AvailableSocket // Channel with available (net.Conn, error
...
// Close down the socket by sending a close message over
// the control channel to the host.
...
// Opens the socket with the given virtual fd. The connection
// and errors will be posted back on the available channel.
...
// Opens a connection without blocking by calling accept
// and sending the identity message over the connection.
func (b *ControlSocket) open(fd uint32) (net.Conn, error) {
...
func (b *ControlSocket) controlSocketHandler() error {
	...
	for {
		...
		// has fixed size! Don't mess with variable encoding
		// lest you want to make this all complicated.
		var fd uint32 = 0
		cl := control_socket_pb.SocketControl_identity
		cinfo := &control_socket_pb.SocketControl{
			Fd:   &fd,
		...
		case control_socket_pb.SocketControl_open:
			...
			// Create, and notify other thread..
